...
echo ""
echo "╔══════════════════════════════════════════╗"
echo "║       Wibo MCP — Instalador v10.0        ║"
echo "╚══════════════════════════════════════════╝"
echo ""
...
for f in config.js db.js store-resolver.js api.js server.js org-list.js; do
  curl -fsSL "$RAW_URL/src/$f" -o "$INSTALL_DIR/src/$f"
done

for f in admin.js stores.js payments.js organizations.js api-commercial.js api-transactions.js api-payments.js api-features.js report-guide.js usage-guide.js; do
